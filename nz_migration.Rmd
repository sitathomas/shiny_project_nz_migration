...
# Age data, not fully functional.
```{r arrivals_by_age, include = F}
...
ggplot(by_visa_gender, aes(x = year, y = arrivals, fill = gender)) +
	geom_col(position = "dodge")

by_visa_gender %>% 
	group_by(., year, visa_type) %>% 
	summarise(., total = sum(arrivals)) %>% 
	ggplot(data = ., aes(x = year, y = total, fill = visa_type)) +
		geom_col() +
		labs(
